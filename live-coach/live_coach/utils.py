...
class RepetitionCounter(object):
  ...
  def __init__(self, class_name = None, enter_threshold=6, exit_threshold=4):
    ...
    self._n_repeats = {'pullups': 0, 'pushups': 0, 'situp': 0, 'squats': 0, 'jumping_jacks': 0, '': 0}

  @property
  def n_repeats(self):
    return self._n_repeats

  def __call__(self, pose_classification, class_name = None):
    ...
    pose_confidence = 0.0
    if class_name != None:
        if class_name == 'jumping_jacks' or class_name == 'pullups':
            self._class_name = class_name + '_up'
        else:
            self._class_name = class_name + '_down'
...
class PoseRecognition(object):
  ...
  def __call__(self, pose_classification):
    ...
    # Get pose confidence.
    pose_confidence = 0.0
    pose_name = max(pose_classification)
    pose_confidence = pose_classification[pose_name]
    ...
    pose_name_gen = self._pose_name.replace('_up', '')
    pose_name_gen = pose_name_gen.replace('_down', '')
    #else:
    #    pose_name_gen = ''
        
    return pose_name_gen
...
class PoseClassificationVisualizer(object):
  ...
  def __call__(self,
    ...
    ###modifs recognition
    output_img_draw.text((output_width * 0.10,
                          output_height * 0.05),
                         pose_name,
                         font=self._counter_font,
                         fill=self._counter_font_color)
...
class BootstrapHelper(object):
  ...
  def __init__(self,
    ...
    self._pose_class_names = None
    
  def bootstrap_images(self, per_pose_class_limit=None):
  ...
  def bootstrap_ldk(self, csv_file, per_pose_class_limit=None):
    """Tentative de production de csv triés par classe
    """
    # Create output folder for CVSs.
    if not os.path.exists(self._csvs_out_folder):
      os.makedirs(self._csvs_out_folder)
    ...
    liste_im = [i for i in range(len(data_csv))]
    data_csv.insert(2, 'image', liste_im)
    self._pose_class_names = data_csv['pose'].unique()
    ...
    for pose_class_name in self._pose_class_names:
      ...
      data_csv[data_csv['pose'] == pose_class_name].iloc[:, 2:].to_csv(csv_out_path, header = False, index = False)
  ...
  def analyze_outliers(self, outliers, ldk = None):
  ...
  def remove_outliers_ldk(self, outliers):
    """Removes outliers from csv files."""
    list_to_remove = []
    for outlier in outliers:
      list_to_remove.append(outlier.sample.name)
    
    for classe in self._pose_class_names:
      Data_path = os.path.join(self._csvs_out_folder, classe + '.csv')
      Data_to_modif = pd.read_csv(Data_path, header = None)
      Data_to_save = Data_to_modif[~Data_to_modif.iloc[:,0].isin(list_to_remove)]
      print('Nombre de données pour la classe ' + classe + ': ' + str(len(Data_to_save)))
      Data_to_save.to_csv(Data_path, header = False, index = False)
  
  def remove_outliers_csv(self, outliers):
    """Removes outliers from the image folders."""
    for outlier in outliers:
      image_path = os.path.join(self._images_out_folder, outlier.sample.class_name, outlier.sample.name)
      os.remove(image_path)
...
def dump_for_the_app():
  pose_samples_folder = '../Data_csv/fitness_poses_csvs_out'
  pose_samples_csv_path = '../Data_csv/fitness_poses_csvs_out.csv'
  file_extension = 'csv'
  file_separator = ','
...
def plot_resultat(dict_:dict):
    resultat = {key:value for (key, value) in dict_.items() if len(key)!=0}
    colors = ['salmon', 'gold', 'limegreen','dodgerblue','navy']
    
    plt.figure(figsize=(8,5))
    plt.bar(resultat.keys(), resultat.values(), color=colors)
    plt.title('Résumé de ta séance : ', fontsize=20)
    plt.xlabel("Exercices", fontsize=15)
    plt.ylabel("Répétitions", fontsize=15)
    plt.savefig('./resume.png')
    im = Image.open('./resume.png')
    im.show()

def entrainement(input_number):
    # Data pour l'entrainement : 
    bootstrap_images_in_folder = '../Data_csv/train.csv'
    # Output folders for bootstrapped images and CSVs.
    bootstrap_images_out_folder = '../Data_csv/fitness_poses_images_out'
    bootstrap_csvs_out_folder = '../Data_csv/fitness_poses_csvs_out'
    ...
    bootstrap_helper.bootstrap_ldk('../Data_csv/train.csv', per_pose_class_limit=None)
    # Check how many images were bootstrapped.
    #bootstrap_helper.print_images_out_statistics()
    ...
    # Analyze outliers.
    #bootstrap_helper.analyze_outliers(outliers)
    bootstrap_helper.analyze_outliers(outliers, ldk = 1)

    # Remove all outliers (if you don't want to manually pick).
    #bootstrap_helper.remove_outliers(outliers)
    bootstrap_helper.remove_outliers_ldk(outliers)
    ...
    with tqdm.tqdm(total=video_n_frames, position=0, leave=True) as pbar:
        while True:
            ...
            if pose_landmarks is not None:
                ...
                # Exercise name
                pose_name = pose_recognizer(pose_classification_filtered)

                # Count repetitions.
                repetitions_count = repetition_counter(pose_classification_filtered, pose_name)
            else:
                # No pose => no classification on current frame.
                pose_classification = None
                # Initialize exercise name to '' if no landmarks detected
                pose_name = ''
            ...
            output_frame = pose_classification_visualizer(
                ...
                repetitions_count=repetitions_count[pose_name],
                pose_name = pose_name)

            # Save the output frame.
            out_video.write(cv2.cvtColor(np.array(output_frame), cv2.COLOR_RGB2BGR))
            if input_path == 'webcam':
              cv2.imshow('webcam', cv2.cvtColor(np.array(output_frame), cv2.COLOR_RGB2BGR))# press escape to exit
        ...
        # Afficheage du nombre par exercice
        plot_resultat(repetitions_count)
